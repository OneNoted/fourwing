// Compile with: g++ fourwing5.cpp -fopenmp -O3 -I /path/to/eigen -o fourwing_parallel_ab
...
int main() {
    ...
    vector<double> a_range = linspace(-1.8, 2.2, 100);
    vector<double> b_range = linspace(-0.09, 0.11, 100);

    MatrixXd a_b_array(100, 100);
        ...
        for (int bi = 0; bi < 100; ++bi) {
            ...
            for (; steps < 100000; ++steps) {
                ...
                for (auto& p : particles) {
                    p.RK4(a_range[ai], b_range[bi], -0.4);
            ...
            #pragma omp critical
            {
                if (percent_complete >= last_reported + 10) {
                    ...
                    cout << "[a_b matrix] " << percent_complete << "% ("
    ...
    saveMatrixToCSV(a_b_array, "a_b_array.csv");
